...
public class ClusterConnection implements StateListener, AutoCloseable
{
     ...
     * per application. Each session maintains multiple connections to the
     * cluster nodes.
     */
    private Session mySession = null;

    /*
     * MappingManager is thread-safe and can be safely shared throughout
     * application. Create one instance at startup, right after the c* session
     * established.
     */
     ...
     * points. Reconnects with new properties if configuration has been changed
     * or just connects if no connection already exists.
     *
     * @param properties The properties contains the contact points to the
     *                   database cluster.
     */
	...
	if (clusterBuilder.getContactPoints().isEmpty())
	{
	    throw new IllegalArgumentException(
	        "Need at least one host to connect to");
	}

	int port = myDBClusterConnectionProperties.getMyContactPointPort();
	LOG.info("Connecting - host: {}, port: {}",
	    myDBClusterConnectionProperties.getMyContactPointAddresses(), port);
	clusterBuilder.withPort(port).withQueryOptions(options)
	    .withLoadBalancingPolicy(loadBalancingPolicy)
	    .withReconnectionPolicy(
	        new ExponentialReconnectionPolicy(1000, 20000))
	    .withRetryPolicy(retryPolicy)
	    .withTimestampGenerator(ServerSideTimestampGenerator.INSTANCE)
	    .withProtocolVersion(protocolVersion).withCredentials(
	        myDBClusterConnectionProperties.getMyServerUsername(),
	...
	myCluster.getCluster()
	    .register(QueryLogger.builder().withConstantThreshold(50).build());
	mySession = myCluster.getCluster().newSession();
	LOG.info("Cluster connection created with protocol version {}",
	    protocolVersion);
    ...
    {
	Set<String> addresses = myDBClusterConnectionProperties
	    .getMyContactPointAddresses();
    ...
    {
	LOG.debug("Number of hosts in contact point list: {}",
	    addresses.size());
    ...
    {
	LOG.info("A new node {0} is added to cluster at DC {1}",
    ...
    {
	LOG.info("A node {0} is getting down at DC {1}", host.getAddress(),
	    host.getDatacenter());
    ...
    {
	LOG.info("A node {0} is removed from DC {1}", host.getAddress(),
	    host.getDatacenter());
	myUpHosts.remove(host);
    }
    ...
    {
	LOG.info("Node {0} is up at DC {1}", host.getAddress(),
	    host.getDatacenter());
	myUpHosts.add(host);
    }
	...
	if (!myCluster.isAvailable())
	{
	    throw new UnableToProcessException("Driver is not initialized!",
	        myClusterConnectionException);
	}
	return mySession;
    }
