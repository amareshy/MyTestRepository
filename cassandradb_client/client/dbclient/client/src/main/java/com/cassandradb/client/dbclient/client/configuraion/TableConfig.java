...
public class TableConfig {
	...
	private TableConfig(TableConfigBuilder builder) {
		if(StringUtils.isEmpty(builder.databaseName)) {
			throw new IllegalArgumentException("databaseName must be set and shouldn't be null or empty");
		}
		if(StringUtils.isEmpty(builder.tableName)) {
			throw new IllegalArgumentException("tableName must be set and shouldn't be null or empty");
		}
		if(CollectionUtils.isEmpty(builder.tableOptions)) {
			throw new IllegalArgumentException("tableOptions must be set and shouldn't be null or empty");
		}
		if(builder.tableType == null || EnumUtils.getEnum(TableType.class, builder.tableType.name()) == null) {
			throw new IllegalArgumentException("tableType must be set and should be a valid value");
		}
